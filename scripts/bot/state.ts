 ...
 * Headless port of frontend/src/hooks/useGame.ts.
 */
...
export class StateReader {
  ...
  constructor(worldAddress: string, toriiUrl: string) {
  ...
  private async fetchModel(model: `${string}-${string}`, keys: string[], limit = 200) {
  ...
  async poll(tableId: number): Promise<GameState> {
    ...
    try {
      ...
      if (handId > 0) {
        ...
        if (ccE.length > 0) {
          communityCards = parseCommunityCards(ccE[0].models?.[NAMESPACE] ?? {});
        }
